...
DDT2Controller::DDT2Controller()
    : controller_interface::ControllerInterface(), rt_command_ptr_(nullptr),
      rt_desired_velocity_ptr_(nullptr), joints_command_subscriber_(nullptr),
      joints_desired_velocity_subscriber_(nullptr) {}

controller_interface::return_type
DDT2Controller::init(const std::string &controller_name) {
...
CallbackReturn DDT2Controller::on_configure(
  ...
  if (command_mode_ != "position" && command_mode_ != "velocity" &&
      command_mode_ != "effort" && command_mode_ != "position_velocity") {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "'command_mode' parameter was error check the .yaml");
    return CallbackReturn::ERROR;
  }
  ...
  joints_desired_velocity_subscriber_ =
      get_node()->create_subscription<CmdType>(
          "~/joints_desired_velocity", rclcpp::SystemDefaultsQoS(),
          [this](const CmdType::SharedPtr msg) {
            rt_desired_velocity_ptr_.writeFromNonRT(msg);
          });
...
DDT2Controller::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
DDT2Controller::state_interface_configuration() const {
...
controller_interface::return_type DDT2Controller::update() {
  ...
  auto joint_desired_velocitys = rt_desired_velocity_ptr_.readFromRT();
  ...
  if (!joint_commands || !(*joint_commands)) {
    ...
    for (auto index = 0ul; index < command_interfaces_.size(); ++index) {
      if (command_mode_ == "effort") {
        output_commands[index] = 0.;
        ...
        output_commands[index] =
            pids_[index]->computeCommand(pos_err, cur_time - last_time_);
      } else if (command_mode_ == "velocity" ||
                 command_mode_ == "position_velocity") {

        double cur_vel;
        for (const auto &state_interface : state_interfaces_) {
          if (state_interface.get_full_name().find("velocity") !=
                  std::string::npos &&
              state_interface.get_full_name().find(
                  command_interfaces_[index].get_name()) != std::string::npos)
        ...
        rclcpp::Time cur_time = rclcpp::Clock().now();
        output_commands[index] =
            pids_[index]->computeCommand(vel_err, cur_time - last_time_);
      }
  ...
  for (auto index = 0ul; index < command_interfaces_.size(); ++index) {
    ...
    if (command_mode_ == "effort") {
      output_commands[index] = original_desired[index];
    } else if (command_mode_ == "position") {
      double cur_pos;
      for (const auto &state_interface : state_interfaces_) {
        if (state_interface.get_full_name().find("position") !=
      ...
      output_commands[index] =
          pids_[index]->computeCommand(vel_err, cur_time - last_time_);
    } else if (command_mode_ == "position_velocity") {
      double cur_vel, cur_pos;
      for (const auto &state_interface : state_interfaces_) {
        if (state_interface.get_full_name().find("velocity") !=
                std::string::npos &&
            state_interface.get_full_name().find(
      ...
      if (!joint_desired_velocitys || !(*joint_desired_velocitys))
        desired_velocity = 1;
      else
        desired_velocity = (*joint_desired_velocitys)->data[index];
      double vel_err = desired_velocity - abs(cur_vel),
             pos_err = original_desired[index] - cur_pos;
      rclcpp::Time cur_time = rclcpp::Clock().now();
      //      double pos_pid_out =
      //          pids_[index]->computeCommand(pos_err, cur_time - last_time_);
      if (abs(pos_err) >= 1)
...
PLUGINLIB_EXPORT_CLASS(ddt2_controller::DDT2Controller,
