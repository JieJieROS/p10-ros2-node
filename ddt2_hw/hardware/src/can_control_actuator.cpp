...
CanControlActuator::configure(const hardware_interface::HardwareInfo &info) {
  ...
  act_coeffs_path_ = info_.hardware_parameters["act_coeffs_path"];
  act_datas_path_ = info_.hardware_parameters["act_datas_path"];
  bus_info_path_ = info_.hardware_parameters["bus_info_path"];
  if (act_coeffs_path_.empty() || act_datas_path_.empty() ||
      bus_info_path_.empty())
    RCLCPP_FATAL_STREAM(
        rclcpp::get_logger("CanControlActuator"),
        "The config path is incomplete, check the `act_coeffs_path`, "
        "`act_datas_path` and `bus_info_path`param "
        "in your controller.yaml");
  control_level_.resize(info_.joints.size(), integration_level_t::EFFORT);

  for (const hardware_interface::ComponentInfo &joint : info_.joints) {
    if (joint.command_interfaces.size() != 1) {
      RCLCPP_FATAL(rclcpp::get_logger("CanControlActuator"),
                   "Actuator '%s' has %d command interfaces. 1 expected.",
                   joint.name.c_str());
      return hardware_interface::return_type::ERROR;
    }
  ...
  can_motor_.init(act_coeffs_path_, act_datas_path_, bus_info_path_);

  return hardware_interface::return_type::OK;
}

std::vector<hardware_interface::StateInterface>
CanControlActuator::export_state_interfaces() {
  std::vector<hardware_interface::StateInterface> state_interfaces;
  for (auto &joint : info_.joints) {
    can_interface::ActData *actData = can_motor_.getActDataByName(
        joint.name + "_motor", can_motor_.bus_id2act_data_);
    state_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_EFFORT,
                                  &(actData->effort));
...
CanControlActuator::export_command_interfaces() {
  std::vector<hardware_interface::CommandInterface> command_interfaces;
  for (auto &joint : info_.joints) {
    can_interface::ActData *actData = can_motor_.getActDataByName(
        joint.name + "_motor", can_motor_.bus_id2act_data_);
    command_interfaces.emplace_back(
        joint.name, hardware_interface::HW_IF_EFFORT, &(actData->exe_cmd));
  }
...
hardware_interface::return_type CanControlActuator::start() {
  ...
  auto pub = node.create_publisher<ddt2_msgs::msg::ActuatorState>(
      "/actuator_states", 100);
  actuator_state_pub_ = std::make_shared<
      realtime_tools::RealtimePublisher<ddt2_msgs::msg::ActuatorState>>(pub);
  return hardware_interface::return_type::OK;
}
...
hardware_interface::return_type CanControlActuator::read() {
  ...
  //        {
  //            try
  //            {  // Duration will be out of dual 32-bit range while motor
  //            failure
  //                act_data.second.halted = (curren -
  //                act_data.second.stamp).seconds() > 0.1 ||
  //                act_data.second.temp > 99;
  //            }
    ...
    if (actuator_state_pub_->trylock()) {
      ...
      for (const auto &id2act_datas : can_motor_.bus_id2act_data_)
        for (const auto &act_data : id2act_datas.second) {
          ...
          actuator_state.need_calibration.push_back(
              act_data.second.need_calibration);
          actuator_state.calibrated.push_back(act_data.second.calibrated);
          actuator_state.calibration_reading.push_back(
              act_data.second.calibration_reading);
          ...
          actuator_state.executed_effort.push_back(act_data.second.exe_cmd);
        }
...
PLUGINLIB_EXPORT_CLASS(ddt2_hw::CanControlActuator,
