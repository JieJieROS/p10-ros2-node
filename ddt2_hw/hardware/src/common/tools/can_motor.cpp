...
namespace can_interface {
    bool CanMotor::init(std::string act_coeffs_yaml, std::string act_datas_yaml,
                        std::string bus_yaml) {
        bool success = true;
    ...
    bool CanMotor::parseActCoeffs(std::string act_coeffs_yaml) {
        ...
        for (auto it = act_coeffs.begin(); it != act_coeffs.end(); ++it) {
            ...
            coeff.act2effort =
                    params["act2effort"] ? params["act2effort"].as<double>() : 0.0;
            coeff.effort2act =
                    params["effort2act"] ? params["effort2act"].as<double>() : 0.0;
            ...
            coeff.pos_offset =
                    params["pos_offset"] ? params["pos_offset"].as<double>() : 0.0;
            coeff.pos_offset_hex =
                    params["pos_offset_hex"] ? params["pos_offset_hex"].as<std::string>() : "0000";
            coeff.vel_offset =
                    params["vel_offset"] ? params["vel_offset"].as<double>() : 0.0;
            coeff.effort_offset =
                    params["effort_offset"] ? params["effort_offset"].as<double>() : 0.0;
    ...
    bool CanMotor::parseActData(std::string act_datas_yaml) {
        ...
        for (auto it = act_datas.begin(); it != act_datas.end(); ++it) {
            ...
            if (!actuator_data["bus"] || !actuator_data["type"] ||
                !actuator_data["id"] || !actuator_data["mode"]) {
                std::cerr << "Actuator " << actuator_name << " has incomplete data."
                          << std::endl;
                continue;
            }
            ...
            std::string mode = actuator_data["mode"].as<std::string>();
            ...
            if (bus_id2act_data_.find(bus) != bus_id2act_data_.end() &&
                ...
                std::cerr << "Duplicate actuator on bus " << bus << " and ID " << id
                          << std::endl;
                return false;
            } else {
                bus_id2act_data_[bus].insert(
                        std::make_pair(id, can_interface::ActData{
    ...
    bool CanMotor::initCanBus(std::string bus_yaml) {
        ...
        try {
            if (!config["bus"]) {
                ...
                return true; // Consider returning false here if no bus specified means
                // failure
            }
            ...
            for (const auto &bus: buses) {
                ...
                if (bus_name.find("can") != std::string::npos) {
                    can_buses_.push_back(new can_interface::CanBus(
                            bus_name,
                            can_interface::CanDataPtr{.type2act_coeffs_ = &type2act_coeffs_,
                                    .id2act_data_ =
                                    &bus_id2act_data_[bus_name]},
                            thread_priority));
                } else {
    ...
    ActData *CanMotor::getActDataByName(
            const std::string &name,
            std::unordered_map<std::string, std::unordered_map<int, ActData>>
            &bus_id2act_data) {
